...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      // Throttle to one update per rAF — prevents scroll jank from mousemove flood
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        ...
        const dx = mouseX - p.x;
        const dy = mouseY - p.y;
        const Math_sqrt = Math.sqrt(dx * dx + dy * dy);
        if (Math_sqrt < 200) {
          p.x += dx * 0.002;
          p.y += dy * 0.002;
        }
        ...
        // Draw particle — no shadowBlur (very expensive on canvas)
  ...
  return (
    <section 
      id="hero" 
      className="relative min-h-screen w-full flex items-center justify-center pt-20 pb-16 overflow-hidden px-4 md:px-16"
    >
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-7 flex flex-col items-center text-center lg:items-start lg:text-left">
          ...
          <h2 className="leading-none text-foreground select-none">
            <motion.span 
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.1 }}
              className="block font-serif-lux italic font-light text-2xl md:text-5xl text-foreground/80 mb-2"
            >
              The sound of
            </motion.span>
            <motion.span 
              ...
              className="block font-brittany text-6xl sm:text-7xl md:text-9xl text-gold tracking-normal normal-case pt-6 pb-4 md:pt-10 md:pb-6 select-text drop-shadow-[0_0_15px_rgba(212,175,55,0.3)] mt-2 md:mt-6"
            >
              Sukoon.....
            </motion.span>
          </h2>
        ...
        {/* Right Side: 3D Vinyl Player Widget & Statically Aligned Bottom Player */}
        <div className="lg:col-span-5 flex flex-col items-center gap-6 relative">
          ...
          {/* 4. ALIGNED GLASS MUSIC PLAYER COMPONENT */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
